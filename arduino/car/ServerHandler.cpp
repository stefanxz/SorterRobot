...
ServerHandler::Request ServerHandler::handleClient()
{
    ...
    ServerHandler::Request request = Request::NONE;
...
ServerHandler::Request ServerHandler::handleGetRequest(WiFiClient &client, String &path)
{
    ...
    if (path == "/ready")
    {
        return handleIsReadyRequest(client);
    }
}
...
ServerHandler::Request ServerHandler::handleIsReadyRequest(WiFiClient &client)
{
    client.println("HTTP/1.1 200 OK");
    client.println("Content-type: text/html");
    client.println("Connection: close");
...
ServerHandler::Request ServerHandler::handleDriveRequest(WiFiClient &client, String &body)
{
    ...
    if (startPos != -1)
    {
        ...
        if (endPos == -1)
        { // No more parameters, end is the length of the body
            endPos = body.length();
        }
        ...
        Serial.print("Extracted Gate: ");
        Serial.println(gate); // Debug: Output the extracted gate value
    }
    else
    {
